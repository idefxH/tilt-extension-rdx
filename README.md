...
|---|---|---|
| `live_reload=True` | `BP_LIVE_RELOAD_ENABLED=true` | Paketo wraps the buildpack's own `web` process in watchexec; the extension selects `--default-process web` and **excludes the Procfile** from the build (a `web:` Procfile entry would otherwise overwrite the watchexec-wrapped `web` and silently break reload). A Tilt file sync then restarts just the app process — the Paketo-native replacement for the heroku nodemon `dev:` hack. Requires an npm `start` script / native entrypoint (the Procfile is excluded). nodejs/python/java only (go's buildpack has no watchexec support; ignored with a warning). If the project ships its own `project.toml`, the Procfile is left alone and a warning explains how to exclude it. watchexec uses inotify by default, which can miss events on some containerd/overlayfs setups — fall back to `live_reload=False` (nodemon) if reloads don't fire. |
